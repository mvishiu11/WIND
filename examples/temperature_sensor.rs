...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            reading.insert("timestamp".to_string(), WindValue::I64(
                std::time::SystemTime::now()
                    .duration_since(std::time::UNIX_EPOCH)
                    .unwrap()
                    .as_secs() as i64
            ));
            reading.insert("sensor_id".to_string(), WindValue::String("TEMP_001".to_string()));
    ...
    while let Some(value) = subscription.next().await {
        ...
        if let WindValue::Map(reading) = value {
            ...
            info!("🌡️  {} | {:.2}°C | Change: {:.2}°C | Samples: {} | Rate: {:.1}/s", 
                sensor_id, 
                temp, 
                change,
                sample_count,
