...
pub async fn discover(registry: &str, pattern: &str, json: bool) -> anyhow::Result<()> {
    ...
    if json {
        ...
        if services.is_empty() {
            ...
            println!("Found {} service(s) matching '{}':", services.len(), pattern);
...
pub async fn subscribe(
    ...
    info!("Subscribing to service '{}' with mode {:?}", service, subscription_mode);
    ...
    println!("Subscribed to '{}'. Waiting for data... (Ctrl+C to stop)", service);
...
pub async fn call(
    ...
    info!("Calling {}::{} with params: {:?}", service, method, wind_params);
...
pub async fn publish(
    ...
    if let Some(count) = repeat {
        println!("Would repeat {} times with {}ms intervals", count, interval_ms);
    }
...
fn json_to_wind_value(json: serde_json::Value) -> WindValue {
    ...
    match json {
        ...
        Value::Array(arr) => {
            WindValue::Array(arr.into_iter().map(json_to_wind_value).collect())
        }
        Value::Object(obj) => {
