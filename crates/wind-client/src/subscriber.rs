...
use wind_core::{
    Message, MessagePayload, WindValue, SubscriptionMode, QosParams, 
    ServiceInfo, Result, WindError
};
...
impl Subscriber {
    ...
    pub async fn subscribe(
        ...
        match ack_msg.payload {
            MessagePayload::SubscribeAck { subscription_id: _ack_id, success, error, current_value } => {
                if !success {
    ...
    pub async fn discover_service(&mut self, service_name: &str) -> Result<ServiceInfo> {
        ...
        match response.payload {
            ...
            MessagePayload::Error { error, .. } => {
                Err(WindError::Registry(error))
            }
            _ => {
                Err(WindError::Protocol("Unexpected response".to_string()))
            }
        }
    }
