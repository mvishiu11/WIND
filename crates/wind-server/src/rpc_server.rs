...
use wind_core::{Message, MessageCodec, MessagePayload, Result, ServiceType, WindError, WindValue};

/// RPC method handler trait - using Box<dyn Fn> instead of async trait for object safety
pub type RpcHandlerFn =
    Box<dyn Fn(WindValue) -> futures::future::BoxFuture<'static, Result<WindValue>> + Send + Sync>;
...
pub trait RpcHandler: Send + Sync {
    fn handle(
        &self,
        params: WindValue,
    ) -> Pin<Box<dyn Future<Output = Result<WindValue>> + Send + '_>>;
}
...
impl RpcServer {
    ...
    where
        ...
        Fut: std::future::Future<Output = Result<WindValue>> + Send + 'static,
    {
        self.register_method(method_name, FunctionHandler::new(handler))
            .await
    }
    ...
    async fn register_service(&self, actual_address: &str) -> Result<()> {
        ...
        match response.payload {
            MessagePayload::ServiceRegistered { success, error, .. } => {
                if success {
                    info!(
                        "Successfully registered RPC service '{}' with registry",
                        self.service_name
                    );
                    Ok(())
                } else {
    ...
    async fn handle_client(
        ...
        loop {
            ...
            match request.payload {
                ...
                _ => {
                    warn!(
                        "Unexpected message type in RPC server: {:?}",
                        request.payload
                    );
                }
            }
