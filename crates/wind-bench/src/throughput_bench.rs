...
pub async fn run(
    ...
    for handle in handles {
        ...
        println!("Subscriber {} received {} messages ({} bytes)", sub_id, msgs, bytes);
    ...
    println!("Total bytes: {} ({:.2} MB)", total_b, total_b as f64 / 1_048_576.0);
    println!("Message rate: {:.0} msgs/sec", total_msgs as f64 / duration);
    println!("Throughput: {:.2} MB/sec", (total_b as f64 / 1_048_576.0) / duration);
    println!("Per-subscriber rate: {:.0} msgs/sec", (total_msgs as f64 / subscribers as f64) / duration);
